...
結婚式後バージョンの **`/photos` ページ**で、ゲストが当日の写真を投稿・閲覧・ダウンロードできます（トップページには誘導ボタンと、写真が入れ替わるアルバムセクションのみ）。保存先は Google Drive です。ギャラリーは **24枚ごとのページ切り替え**（‹ 1 / N ›）。写真の角の◯をタップすると選択でき、画面下の追従バーから**選択した写真だけを ZIP 保存**できます。拡大表示は左右スワイプでも送れます。

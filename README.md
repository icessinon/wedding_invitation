...
2. **ビルドと起動（ローカル・ポート競合を避ける場合）**
   ...
   **t3a.medium（メモリ 4GB）などでビルドが OOM や CPU 使い果たしになる場合**は、ビルド時にメモリ・CPU を制限する。
   ...
   ```bash
   DOCKER_BUILDKIT=1 docker build --memory=2g --cpus=2 -t ga4-analytics-dashboard:latest -f Dockerfile .
   docker compose up -d
   ```
   ...
   - `--memory=2g` でビルドコンテナ全体のメモリ上限、`--cpus=2` で CPU 使用を制限し、ホストがハングしないようにします。
